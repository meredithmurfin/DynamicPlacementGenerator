...
def import_engine_subtypes():
	logging.info("This program can generate optimal spare placement for the following engine subtypes:")
	with open('data_to_read/engine_subtypes.csv', 'rt') as file:
		data_from_file = csv.reader(file)
		rows = list(data_from_file)[1:]
		for row in rows:
			data.engine_subtypes.append(row[0])
			logging.info(row[0])
	...
	with open(filepath, 'rt') as file:
		...
		for row in all_rows:
			...
			try:
				...
				assert (row[11].upper() in ['TRUE', 'FALSE']), "The value in column 12 of the removal_info file must either be TRUE, indicating the information in the file has been changed since the previous run, or FALSE, indicating the information in the file is the same as the previous run."
			except Exception as e:
				logging.error("An exception has occurred: " + e)
				raise Exception("POSSIBLE SOLUTION: Make sure you are inputting integer values into the removal_info file for columns 2 through 11. String values cannot be accepted.")
...
def import_engine_info(filepath, data_storage):
	...
	with open(filepath, 'rt') as file:
		...
		all_rows = data_as_list[1:]
		for row in all_rows:
			assert ('' not in row[:12]), "There are empty cells in the engine_info file. All information for each fleet must be inputted. Values may be zero, but they may not be blank."
			try:
				engine_subtype = row[0]
				...
				logging.error("An exception has occurred: " + e)
				raise Exception("POSSIBLE SOLUTION: Make sure you are inputting integer values into the engine_info file for columns 2 through 11. String values cannot be accepted.")
...
def import_num_departures_by_hub_monthly(filepath, engine_subtype, data_storage):
	...
	with open(filepath, 'rt') as file:
		...
		try:
			...
			logging.error("An exception has occurred: " + e)
			raise Exception("POSSIBLE SOLUTION: Make sure the data in the num_departures_by_hub_monthly file for the " + engine_subtype + " is organized with 3 rows, each row providing counts of departures for that month at that location.")
		for i in range(len(data.hubs) + 1):
			hub = hubs_header[i]
			assert ((hub in data.hubs) or (hub in ['OTHER'])), "The header in the num_departures_by_hub_monthly file for the " + engine_subtype + " must be organized by each valid hub with one column for OTHER."
...
def import_total_departures_ground_time_by_state_region_monthly(filepath, engine_subtype, data_storage):
	...
	with open(filepath, 'rt') as file:
		...
		try:
			...
			logging.error("An exception has occurred: " + e)
			raise Exception("POSSIBLE SOLUTION: Make sure the data in the total_departures_ground_time_by_state_region_monthly file for the " + engine_subtype + " is organized with 3 rows, each row providing the total ground time from departures for that month at that location.")
		for i in range(len(data.state_regions)):
			state_region = state_regions_header[i]
			assert (state_region in data.state_regions), "The header in the total_departures_ground_time_by_state_region_monthly file for the " + engine_subtype + " must be organized by each valid state region."
...
def import_total_RONSRADS_ground_time_by_hub_monthly(filepath, engine_subtype, data_storage):
	...
	with open(filepath, 'rt') as file:
		...
		try:
			...
			logging.error("An exception has occurred: " + e)
			raise Exception("POSSIBLE SOLUTION: Make sure the data in the total_RONSRADS_ground_time_by_hub_monthly file for the " + engine_subtype + " is organized with 3 rows, each row providing the total ground time from RONs/RADs for that month at that location.")
		for i in range(len(data.hubs) + 1):
			hub = hubs_header[i]
			assert ((hub in data.hubs) or (hub in ['OTHER'])), "The header in the total_RONSRADS_ground_time_by_hub_monthly file for the " + engine_subtype + " must be organized by each valid hub with one column for OTHER."
			data_storage[engine_subtype][hub] = {1: int(month_1[i]), 2: int(month_2[i]), 3: int(month_3[i])}
	return data_storage
